...
Plugin Name: the hippo button

Plugin URI: http://placeholder.com

Description: its the hippo button again.. no escape...
...
add_shortcode('hippobtn', 'hippo_code');

function hippo_plugin_scripts() {
  wp_enqueue_script('jquery-hippo-plugin-script', plugin_dir_url(__FILE__) . 'js/jquery.min.js');

  wp_enqueue_script('hippo-plugin-script', plugin_dir_url(__FILE__) . 'js/custom.js');

  wp_enqueue_script('hippo-js-cdn', plugin_dir_url(__FILE__) . 'js/cdn.js');

  // wp_enqueue_script('hippo-js-cdn', 'https://cdn.jsdelivr.net/npm/gsap@3.0.1/dist/gsap.min.js');
}

add_action('wp_enqueue_scripts','hippo_plugin_scripts');

wp_enqueue_style('hippo-plugin-css', plugin_dir_url(__FILE__) . 'css/custom.css');
