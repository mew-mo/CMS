...
function my_first_customise_option($wp_customize) {
  ...
  $wp_customize->add_control('my_custom_number', array(
    ...
    // input attributes
    // add colourpicker setting - 271021
    // ===========
    $wp_customize->add_setting('color_picker', array(
      'default' => ''
      // this sets the default colour
      // NO SEMICOLONS IN THE ARRAY OR ELSE >:0
    ));

    $wp_customize->add_control(new WP_Customize_Color_Control(
      $wp_customize, 'color_picker', array(
        'label' => 'Link Colours',
        'section' => 'mos_section',
        'settings' => 'color_picker'
      )
    ));
    // wp customizeoclourcontrol is the wp object, has to be called this way . wp knows its a colour picker thanks to this object

    // card bg colourpicker
    $wp_customize->add_setting('color_picker_cardbg', array(
      'default' => ''
    ));

    $wp_customize->add_control(new WP_Customize_Color_Control(
      $wp_customize, 'color_picker_cardbg', array(
        'label' => 'Card Background Colour',
        'section' => 'mos_section',
        'settings' => 'color_picker_cardbg'
      )
    ));

    // trying to add a dropdown!
    // ==============
    $wp_customize->add_setting('dropdown_time', array(
      'default' => ''
    ));

    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'dropdown_time', array(
      'label' => 'Select your thing',
      'description' => 'you will select a thing',
      'settings' => 'dropdown_time',
      'priority' => 10,
      'section' => 'mos_section',
      'type' => 'select',
      'choices' => array(
    ...
    $wp_customize->add_setting('custom_img', array(
      'default' => ''
    ));

    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'custom_img', array(
...
// creating more customizer stuff! img, colourpicker - 271021
// ======================================================
// https://developer.wordpress.org/reference/classes/wp_customize_color_control/

// create new function for generating css that will link to tha colourpicker
function generate_special_css() {
  $color_picker = get_theme_mod('color_picker');
  $bg_picker = get_theme_mod('color_picker_cardbg');
  $dropdown_destruction = get_theme_mod('dropdown_time');
  ?>
  <!-- html the function will return >:D -->
  <style type="text/css">

    * {
      color: <?php echo $dropdown_destruction ?>;
    }
...
// running in the head bc the head loads stylesheet; this is Logical

// adding image upload to the website! heckyeah man! -w-)b
// https://developer.wordpress.org/reference/classes/wp_customize_image_control/

function img_customize_section($wp_customize) {
  $wp_customize->add_section('img_section', array(
    'title' => 'upload ur IMAGEZ', 'custom_setting',
    'priority' => 0
  ));
...
add_action('customize_register', 'img_customize_section');

function img_css() {
  $bg_img = get_theme_mod('custom_bg_img');
  ?>
  <style type="text/css">
